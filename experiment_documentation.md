...
このリポジトリ は、`each_mask/` 内の各時系列データが実データらしいか、生成データらしいかを判別するための異常検知ベースの実験である。
...
このリポジトリ では、Mahalanobis距離が小さい順に以下の4件が並んだ。
...
さらに、参照元の実データwindow自身についても Mahalanobis距離を計算し、各 `each_mask` がその距離分布の中でどこに位置するかを評価している。

参照windowの距離分布は以下であった。
...
実データwindowであっても、平均ベクトルと完全一致しない限り Mahalanobis距離は0にならない。したがって重要なのは、距離の絶対値ではなく、判定対象の距離が実データwindowの距離分布の中でどの位置にあるかである。

`mask2_brown`, `mask4_brown`, `mask2_sabr`, `mask5_sabr` は参照windowの距離分布内に収まっている。一方で、それ以外の6件は参照windowの最大距離 `7.107789` を超えており、実データ分布から見て明確に外側に位置している。

```text
mask2_brown.csv  distance = 4.018166  percentile = 36.45  upper_tail = 0.644860
mask4_brown.csv  distance = 4.033254  percentile = 38.32  upper_tail = 0.616822
mask2_sabr.csv   distance = 4.794737  percentile = 78.50  upper_tail = 0.214953
mask5_sabr.csv   distance = 4.966569  percentile = 84.11  upper_tail = 0.158879
```

ここで `percentile` は、参照windowのうち対象mask以下の距離を持つものの割合である。`upper_tail` は、参照windowのうち対象mask以上の距離を持つものの割合である。

例えば `mask5_sabr.csv` は距離だけを見ると `4.966569` でやや大きく見えるが、実データ参照windowの中でも上位約16%程度では起こる距離であり、実データらしい範囲にあると解釈できる。
...
ただし、Mahalanobis距離は「実データ平均からの距離」であるため、実データwindowであっても距離は0にならない。距離が0になるのは、特徴量ベクトルが平均ベクトル `μ` と完全に一致する場合だけである。

そのため、本実験では `each_mask` の距離だけでなく、参照window自身の距離分布も計算する。

```text
reference_percentile = P(D_ref <= D_mask)
empirical_upper_tail_probability = P(D_ref >= D_mask)
```

これにより、各maskが実データ距離分布の中央付近にいるのか、上側tailにいるのか、あるいは参照windowの最大距離を超えて外側にいるのかを確認できる。

## 6. 実装の流れ

実装は [run_mahalanobis_remake1.py](scripts/run_mahalanobis_remake1.py) にある。
...
9. 参照window自身の Mahalanobis距離分布を計算する
10. 各maskの距離が参照距離分布内で何パーセンタイルかを計算する
11. 距離CSV、特徴量CSV、z-score CSV、分布内位置CSV、SVGグラフ、summaryを出力する
```
...
Hotelling's T² 評価は [run_hotelling_t2.py](hotelling_t2/scripts/run_hotelling_t2.py) にある。
